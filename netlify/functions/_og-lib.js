...
function buildArticleHead(article, ctx) {
   ...
   *
   *  ১) সংবাদের নিজস্ব ছবি — /img/<file> (আমাদের নিজের স্টোরেজে রাখা)।
   *     চাকরির বিজ্ঞপ্তির পোস্টার বা সংবাদের আসল ছবি। ব্যবহারকারীর
   *     চাওয়া: "নিউজের সাথে যে ছবিটি ছিল সেটাই" যাবে।
   *  ২) না থাকলে Oracle-এর তৈরি ১২০০x৬৩০ ব্র্যান্ডেড কার্ড
   *     (/og/<slug>.jpg, /og/* প্রক্সির মাধ্যমে)।
   *
   * কেন সোর্স সাইটের ছবির ঠিকানা সরাসরি ব্যবহার করা হয় না:
   *   বেশিরভাগ সংবাদমাধ্যম হটলিংক ব্লক করে (bd-journal, jugantor,
   *   somoynews, ittefaq, banglatribune → HTTP 403 যাচাইকৃত)। ফলে
   *   ফেসবুকের কার্ডে ছবি আসত না। সার্ভার ছবিটি নিজে নামিয়ে /img/-এ
   *   রাখে, তাই কখনো ৪০৪/৪০৩ হয় না।
   */
  const ownRel = String(article.image || article.og_image || '');
  /* কেবল আমাদের নিজের /img/ স্টোরেজের ছবি গ্রহণ করা হয় — সোর্স সাইটের
     বাইরের ঠিকানা হটলিংক ব্লকের (৪০৩) কারণে বাদ দেওয়া হয়। */
  const ownAbs = /^\/img\//.test(ownRel) ? `${origin}${ownRel}` : '';
