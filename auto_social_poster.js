...
function postFacebook(message, link) {
  ...
  const req = https.request({
    ...
    headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload) }
  }, (res) => { console.log(`Facebook Status: ${res.statusCode}`); });
  req.write(payload); req.end();
}
...
async function runAutoPost() {
  try {
    ...
    let lastPosted = {};
    if (fs.existsSync(postedFile)) lastPosted = JSON.parse(fs.readFileSync(postedFile, 'utf8'));
    ...
    console.log("Posting news to Telegram and Facebook...");
    sendTelegram(msg);
  ...
  } catch (err) { console.error("Auto-post error:", err); }
}
